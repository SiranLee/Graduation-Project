...
class FileManager(models.Manager):
    ...
    def createFile(self, describe, url, title, filename, download_times, not_available2all, tno, sno, no, vtime=time.strftime("%Y-%m-%d %H:%M:%S",time.localtime(time.time())), isDelete=False):
        stu = self.model()
        #print(type(grade))
        # stu.fno = fno
